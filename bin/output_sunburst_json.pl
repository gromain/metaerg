...
open( OUT_C, ">$prefix.count.json" )
  or die "Could not open $prefix.count.json to write, $!\n";
...
#this is for taxonomic, taxon database is messing, sometimes, one order is assigned to different class.
        next
          if $item =~
/unclassified|\-\[class\]|Terrabacteria-group|Cyanobacteria\/Melainabacteria-group|-group/i;
        $item =~ s/^\"//;
        $item =~ s/\"$//;
...
sub getKids {
    ...
    my $current_count_pct =
      sprintf( "%.2f", $current_count * 100 / $root_count );
    ...
    if (@kids) {
        ...
        if ( exists $terms{$current}->{count} ) {
            print OUT_C ' ' x $spaceoffset, "\"size\": ",
              $terms->{$current}->{count}, ",\n";
        ...
        foreach my $kid (@kids) {
            ...
            getKids( $leading + 1, $kid, $parent_kid, $terms,
                $spaceoffset + 2 );
