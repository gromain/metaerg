...
"Could not fetch ftp://ftp.uniprot.org/pub/databases/uniprot/current_release/knowledgebase/complete/$file, $!\n";
    ...
    if ( !-e "$diamond_dir/uniprot_sprot.dmnd" ) {
        my $cmd =
"diamond makedb --tmpdir /dev/shm --in $tmp_dir/uniprot_sprot.fasta.gz -d $diamond_dir/uniprot_sprot";
    ...
    if ( !-e "$tmp_dir/uniprot_sprot.dat" ) {
        ...
        gunzip "$tmp_dir/uniprot_sprot.dat.gz" => "$tmp_dir/uniprot_sprot.dat"
...
#runcmd($cmd);
    ...
    if ( !-e "$tmp_dir/sprot.sqldb.tsv" ) {
        ...
        parse_uniprot_sport_dat( "$tmp_dir/uniprot_sprot.dat",
            "$tmp_dir/sprot.sqldb.tsv" );
    }
}
            ...
            for my $de ( $entry->DEs->elements ) {
                ...
                foreach my $ince (@inces) {
                    foreach my $ide ( $ince->elements ) {
                        ...
                        elsif ($ide->type eq 'Full'
                            && $ide->category eq 'RecName' )
                        {
                            $desc .= ";" . $ide->text;
                        }
                    }
                ...
                if ( $de->type eq 'EC' ) {
                    ...
                    $desc = $de->text;
                    if ( $desc =~
                        m/^UPF\d|^Uncharacterized protein|^ORF|^Protein / )
                    {
                        $desc = $HYPO;
                    }
                }
...
sub build_pfam_db {
    ...
    if ( !-e "$tmp_dir/Pfam-A.hmm.dat" ) {
        ...
        my $ae =
          Archive::Extract->new( archive => "$tmp_dir/Pfam-A.hmm.dat.gz" );
...
sub build_tigrfam_db {
    ...
    my @files = (
        ...
        "TIGRFAMs_15.0_INFO.tar.gz", "TIGR_ROLE_NAMES",
        "TIGRFAMS_ROLE_LINK",        "RELEASE_NOTE_15.0"
    );
    ...
    if ( !-e "$protein_hmm_dir/TIGRFAMs.hmm" ) {
        ...
        my $ae = Archive::Extract->new(
            archive => "$tmp_dir/TIGRFAMs_15.0_HMM.LIB.gz" );
    ...
    if ( !-e "$tmp_dir/TIGRFAMs_15.0_INFO" ) {
        ...
        my $ae = Archive::Extract->new(
            archive => "$tmp_dir/TIGRFAMs_15.0_INFO.tar.gz" );
        $ae->extract( to => "$tmp_dir/TIGRFAMs_15.0_INFO" );
    }
    if ( !-e "$tmp_dir/TIGRFAMs.hmm.sqldb.tsv" ) {
        tigrfam_id2info_table(
            "$protein_hmm_dir/TIGRFAMs.hmm",
            "$tmp_dir/TIGRFAMS_ROLE_LINK",
            "$tmp_dir/TIGR_ROLE_NAMES",
            "$tmp_dir/TIGRFAMs.hmm.sqldb.tsv"
        );
    }
    if ( !-e "$tmp_dir/TIGRFAMS2GO.sqldb.tsv" ) {
        tigrfam_id2go_table( "$tmp_dir/TIGRFAMS_GO_LINK",
            "$tmp_dir/TIGRFAMS2GO.sqldb.tsv" );
    }
}
...
sub tigrfam_id2info_table {
    ...
    foreach ( sort keys %fams ) {
        ...
        if ( exists $fams{$_}->{role_id} ) {
            ...
            $mainrole =
              ( exists $roleid2role{$roleid}->{"mainrole"} )
              ? $roleid2role{$roleid}->{"mainrole"}
              : "";
            $sub1role =
              ( exists $roleid2role{$roleid}->{"sub1role"} )
              ? $roleid2role{$roleid}->{"sub1role"}
              : "";
            $mainrole =~ s/\"/\"\"/g;
            $sub1role =~ s/\"/\"\"/g;
        }
...
sub build_foam_hmmdb {
      ...
      )
    {
        ...
        gunzip "$tmp_dir/FOAM-hmm_rel1a.hmm.gz" =>
          ...
          or die "gunzip failed: $GunzipError\n";
        my $cmd =
"$^X $bin/split_hmm.pl $protein_hmm_dir/FOAM-hmm_rel1a.hmm 5 $protein_hmm_dir FOAM";
...
sub build_genomedb {
    ...
    if ( !-e "$diamond_dir/genomedb.dmnd" ) {
        ...
        # build a File::Fetch object
        my $ff = File::Fetch->new(
            uri => "http://ebg.ucalgary.ca/metaerg/genomedb.faa.gz" );
        msg("Fetching http://ebg.ucalgary.ca/metaerg/genomedb.faa.gz");

        #fetch the uri to local directory
        my $where = $ff->fetch( to => $tmp_dir ) or die $ff->error;
        my $cmd .=
"diamond makedb --tmpdir /dev/shm --in $tmp_dir/genomedb.faa.gz -d $diamond_dir/genomedb";
        msg("Start running:$cmd");
        runcmd($cmd);
    }
    if ( !-e "$tmp_dir/genomedb.tax.tsv" ) {
        ...
        # build a File::Fetch object
        my $ff = File::Fetch->new(
            uri => "http://ebg.ucalgary.ca/metaerg/genomedb.tax.tsv" );
...
sub build_metabolic_hmmdb {
    ...
    if ( !-e "$protein_hmm_dir/metabolic.hmm" ) {
        ...
        # build a File::Fetch object
        my $ff = File::Fetch->new(
            uri => "http://ebg.ucalgary.ca/metaerg/metabolic.hmm.gz" );
        msg("Fetching http://ebg.ucalgary.ca/metaerg/metabolic.hmm.gz");

        #fetch the uri to local directory
        my $where = $ff->fetch( to => $tmp_dir ) or die $ff->error;
        msg("Uncompressing $tmp_dir/metabolic.hmm.gz");
        gunzip "$tmp_dir/metabolic.hmm.gz" => "$protein_hmm_dir/metabolic.hmm"
          or die "gunzip failed: $GunzipError\n";
...
sub build_casgene_hmmdb {
    ...
    if ( !-e "$protein_hmm_dir/casgenes.hmm" ) {
        ...
        # build a File::Fetch object
        my $ff = File::Fetch->new(
            uri => "http://ebg.ucalgary.ca/metaerg/casgenes.hmm.gz" );
        msg("Fetching http://ebg.ucalgary.ca/metaerg/casgenes.hmm.gz");

        #fetch the uri to local directory
        my $where = $ff->fetch( to => $tmp_dir ) or die $ff->error;
        msg("Uncompressing $tmp_dir/casgenes.hmm.gz");
        gunzip "$tmp_dir/casgenes.hmm.gz" => "$protein_hmm_dir/casgenes.hmm"
          or die "gunzip failed: $GunzipError\n";
    }
}
...
sub build_sqlite_db {
    ...
    if ( !-e "$sqlite_dir/metaerg.db" ) {
        ...
        my @table_input_file_names = (
            "pfam2go.sqldb.tsv",     "Pfam-A.hmm.sql.db.tsv",
            "TIGRFAMS2GO.sqldb.tsv", "TIGRFAMs.hmm.sqldb.tsv",
            "go.sqldb.tsv",          "sprot.sqldb.tsv",
            "genomedb.tax.tsv"
        );
        ...
        msg(
"Start copy sql table file sqlfile=$sqlfile, path=$path, filename=$sqlfilename"
        );
...
### fetch the uri to local directory###
    ...
    my @rna_types = (
        "5S_rRNA",           "5_8S_rRNA",
        "SSU_rRNA_bacteria", "SSU_rRNA_archaea",
        "SSU_rRNA_eukarya",  "LSU_rRNA_archaea",
        "LSU_rRNA_bacteria", "LSU_rRNA_eukarya"
    );
    ...
    foreach ( keys %queries ) {
        open $rna_align_filehandlers{$_}, ">", $queries{$_}
    ...
    $cmd =
"cat $tmp_dir/arc_16SrRNA.hmm $tmp_dir/arc_23SrRNA.hmm $tmp_dir/arc_5SrRNA.hmm > $rna_hmm_dir/arc.hmm;";
    $cmd .=
"cat $tmp_dir/bac_16SrRNA.hmm $tmp_dir/bac_23SrRNA.hmm $tmp_dir/bac_5SrRNA.hmm > $rna_hmm_dir/bac.hmm;";
    $cmd .=
"cat $tmp_dir/euk_18SrRNA.hmm $tmp_dir/euk_28SrRNA.hmm $tmp_dir/euk_5_8SrRNA.hmm $tmp_dir/euk_5SrRNA.hmm > $rna_hmm_dir/euk.hmm;";
...
#my $ff = File::Fetch->new(uri => "https://www.arb-silva.de/fileadmin/silva_databases/current/Exports/$file");
            my $ff =
              File::Fetch->new( uri =>
"https://www.arb-silva.de/fileadmin/silva_databases/release_$ssu_version/Exports/$file.gz"
              );
            ### fetch the uri to local directory###
            msg("Fetching $file");
            my $where = $ff->fetch( to => $tmp_dir ) or die $ff->error;
...
"get lsu DNA file from $tmp_dir/$lsu to $blast_dir/silva_LSURef.fasta for blast search database making"
    ...
    if ( !-e "$blast_dir/silva_SSURef_Nr99.fasta.nin" ) {
        my $cmd =
"makeblastdb -input_type fasta -dbtype nucl  -in $blast_dir/silva_SSURef_Nr99.fasta;";
        msg("Start running:$cmd");
        runcmd($cmd);
    }
    if ( !-e "$blast_dir/silva_LSURef.fasta.nin" ) {
        my $cmd =
"makeblastdb -input_type fasta -dbtype nucl  -in $blast_dir/silva_LSURef.fasta";
        msg("Start running:$cmd");
        runcmd($cmd);
    }
}
