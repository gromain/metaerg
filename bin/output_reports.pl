...
my $keggs =
  predict_kegg_pathways( "$datadir/cds.gene2ko", $ko2genes, \%seqHash );
my $metacycs =
  predict_metacyc_pathways( "$datadir/cds.gene2ec", $ec2genes, \%seqHash );
...
sub output_htmlreport {
    ...
    dircopy( "$templatedir/html", "$outdir/html", )
      or die "Copy $templatedir/html to $outdir/html failed, $!\n";
    dircopy( "$templatedir/images", "$outdir/images" )
      or die "Copy $templatedir/images to $outdir/images failed, $!\n";
    dircopy( "$templatedir/js", "$outdir/js" )
      or die "Copy $templatedir/js to $outdir/js failed, $!\n";
    copy "$templatedir/help.html", "$outdir"
      or die "Copy $templatedir/help.html to $outdir/help.html failed, $!\n";
    copy "$templatedir/index.html", "$outdir"
      or die "Copy $templatedir/index.html to $outdir/index.html failed, $!\n";
    copy "$templatedir/style.css", "$outdir"
      or die "Copy $templatedir/style.css to $outdir/style.css failed, $!\n";
...
sub output_master_annot_summary {
   ...
   # Write it all out!
    ...
    my @tags = (
        "casgene_acc",  "sp",        "tm_num", "sprot_desc",
        "tigrfam_desc", "pfam_desc", "genomedb_OC"
    );
    ...
    for
      my $sid ( sort { $seqHash{$b}{DNA}->length <=> $seqHash{$a}{DNA}->length }
        keys %$seqHash )
    {
            ...
            @{ $seqHash->{$sid}{FEATURE} } )
        {
            ...
            if ( exists $gene2pathways{$geneid}->{KEGG} ) {
                push( @kegg_pathways,
                    keys %{ $gene2pathways{$geneid}->{KEGG} } );
            }
            if ( exists $gene2pathways{$geneid}->{metacyc} ) {
                push( @metacyc_pathways,
                    keys %{ $gene2pathways{$geneid}->{metacyc} } );
            }
            print $tbl_fh "$sid\t";
            print $tbl_fh ( $f->get_tag_values("ID") )[0], "\t"
              if $f->has_tag("ID");
   ...
   # Output a general .txt file with statistics about the annotation
        ...
        keys %$seqHash )
    {
        ...
        for my $f ( @{ $seqHash{$sid}{FEATURE} } ) {
            ...
            $count{ $f->primary_tag }->{tigrfam}++
              if $f->has_tag('tigrfam_desc');
            $count{ $f->primary_tag }->{pfam}++  if $f->has_tag('pfam_desc');
            $count{ $f->primary_tag }->{sprot}++ if $f->has_tag('sprot_desc');
            $count{ $f->primary_tag }->{genomedb}++
              if $f->has_tag('genomedb_OC');
            $count{ $f->primary_tag }->{metabolic}++
              if $f->has_tag('metabolic_process');
            $count{ $f->primary_tag }->{rRNA_taxon}++
    ...
    for my $ft ( sort keys %count ) {
        ...
        if ( $ft eq "CDS" ) {
            $gene_annot_str .=
              ( not exists $count{$ft}->{sprot} )
              ? "$ft swissprot_hit:\t0\n"
              : "$ft swissprot_hit:\t" . $count{$ft}->{sprot} . "\n";
            $gene_annot_str .=
              ( not exists $count{$ft}->{pfam} )
              ? "$ft pfam_hit:\t0\n"
              : "$ft pfam_hit:\t" . $count{$ft}->{pfam} . "\n";
            $gene_annot_str .=
              ( not exists $count{$ft}->{tigrfam} )
              ? "$ft tigrfam_hit:\t0\n"
              : "$ft tigrfam_hit:\t" . $count{$ft}->{tigrfam} . "\n";
            $gene_annot_str .=
              ( not exists $count{$ft}->{foam} )
              ? "$ft foam_hit:\t0\n"
              : "$ft foam_hit:\t" . $count{$ft}->{foam} . "\n";
            $gene_annot_str .=
              ( not exists $count{$ft}->{metabolic} )
              ? "$ft metabolic_marker_hit:\t0\n"
              : "$ft metabolic_marker_hit:\t" . $count{$ft}->{metabolic} . "\n";
            $gene_annot_str .=
              ( not exists $count{$ft}->{genomedb} )
              ? "$ft genomedb_hit:\t0\n"
              : "$ft genomedb_hit:\t" . $count{$ft}->{genomedb} . "\n";
        }
        elsif ( $ft =~ /SrRNA/ ) {
            $gene_annot_str .=
              ( exists $count{$ft}->{rRNA_taxon} )
              ? "$ft SILVA database search hits:\t"
              . $count{$ft}->{rRNA_taxon} . "\n"
...
sub output_fasta {
    ...
    my $ffn_fh =
      Bio::SeqIO->new( -file => ">$datadir/cds.ffn", -format => 'fasta' );
    my $faa_fh =
      Bio::SeqIO->new( -file => ">$datadir/cds.faa", -format => 'fasta' );
    my $crispr_fh =
      Bio::SeqIO->new( -file => ">$datadir/crispr.ffn", -format => 'fasta' );
    my $tRNA_fh =
      Bio::SeqIO->new( -file => ">$datadir/tRNA.ffn", -format => 'fasta' );
    my $s5_fh =
      Bio::SeqIO->new( -file => ">$datadir/5SrRNA.ffn", -format => 'fasta' );
    my $s16_fh =
      Bio::SeqIO->new( -file => ">$datadir/16SrRNA.ffn", -format => 'fasta' );
    my $s18_fh =
      Bio::SeqIO->new( -file => ">$datadir/18SrRNA.ffn", -format => 'fasta' );
    my $s23_fh =
      Bio::SeqIO->new( -file => ">$datadir/23SrRNA.ffn", -format => 'fasta' );
    my $s28_fh =
      Bio::SeqIO->new( -file => ">$datadir/28SrRNA.ffn", -format => 'fasta' );
            ...
            @{ $seqHash->{$sid}{FEATURE} } )
        {
            ...
            if ( $f->primary_tag eq 'CDS' ) {
                if ( $f->has_tag('sprot_desc') ) {
                    $desc =
                      $f->has_tag('sprot_desc') ? TAG( $f, 'sprot_desc' ) : "";
                }
                elsif ( $f->has_tag('tigrfam_desc') ) {
                    $desc =
                      $f->has_tag('tigrfam_desc')
                      ? TAG( $f, 'tigrfam_desc' )
                      : "";
                }
                elsif ( $f->has_tag('pfam_desc') ) {
                    $desc =
                      $f->has_tag('pfam_desc') ? TAG( $f, 'pfam_desc' ) : "";
                }
                else {
                ...
                my $lineage =
                  $f->has_tag('genomedb_OC') ? TAG( $f, 'genomedb_OC' ) : "";
                my $os = "UNKNOWN";
            ...
            if ( $f->primary_tag =~ /repeat_region/ ) {
                my $rptSeq =
                  $f->has_tag("rpt_unit_seq")
                  ? ( $f->get_tag_values("rpt_unit_seq") )[0]
                  : "";
                $p->desc("/numRepeat=$score /rpt_unit_seq=$rptSeq /cid=$sid")
                  if defined $p;
            ...
            if ( $f->primary_tag =~ /18SrRNA/ ) {
                ...
                $desc .= " \/rRNA_taxon=";
                $desc .=
                  $f->has_tag('rRNA_taxon') ? TAG( $f, 'rRNA_taxon' ) : "";
...
sub output_geneAnnotation {
    ...
    my $types =
      "tRNA:crispr:rRNA:sprot:tigrfam:pfam:casgene:metabolic:genomedb:ec:ko:go";
    ...
    my @element_header =
      ( "Gene", "Contig", "Start", "End", "Gene_Length", "Strand", "Feature" );
    my @common_header =
    ...
    foreach my $type (@annots) {
        ...
        if ( $type =~ /tRNA|crispr|rRNA/ ) {
            open $annot_tab_file_handler{$type}, ">", "$outdir/$type.tab.txt"
              or die "Could not open $outdir/$type.tab.txt to write, $!\n";
            open $annot_json_file_handler{$type}, ">",
              "$outdir/$type.datatable.json"
              or die
              "Could not open $outdir/$type.datatable.json to write, $!\n";
        }
        else {
            open $annot_tab_file_handler{$type}, ">",
              "$outdir/cds.gene2$type.tab.txt"
              or die
              "Could not open $outdir/cds.gene2$type.tab.txt to write, $!\n";
            if ( $type eq "ko" ) {
                open $annot_tab_file_handler{"gene2ko"}, ">",
                  "$outdir/cds.gene2ko.mapping.txt"
                  or die
"Could not open $outdir/cds.gene2ko.mapping.txt to write, $!\n";
            }
            if ( $type eq "ec" ) {
                open $annot_tab_file_handler{"gene2ec"}, ">",
                  "$outdir/cds.gene2ec.mapping.txt"
                  or die
"Could not open $outdir/cds.gene2ec.mapping.txt to write, $!\n";
            }
        }
        ...
        if ( $type eq "crispr" ) {
            ...
            print { $annot_tab_file_handler{"crispr"} } "#",
              join( "\t", ( @element_header, @header ) ), "\n";
        }
        elsif ( $type eq "tRNA" ) {
            push( @header, "tRNA-isoacceptor-anticodon" );
            print { $annot_tab_file_handler{"tRNA"} } "#",
              join( "\t", ( @element_header, @header ) ), "\n";

        }
        elsif ( $type eq "rRNA" ) {
            push( @header, "Taxonomy" );
            print { $annot_tab_file_handler{"rRNA"} } "#",
              join( "\t", ( @element_header, @header ) ), "\n";
        }
        elsif ( $type eq "sprot" ) {
            ...
            push( @header, "Taxonomy" );
            print { $annot_tab_file_handler{"sprot"} } "#",
              join( "\t", ( @common_header, @header ) ), "\n";
        }
        elsif ( $type eq "pfam" ) {
            ...
            push( @header, "Taxonomy" );
            print { $annot_tab_file_handler{"pfam"} } "#",
              join( "\t", ( @common_header, @header ) ), "\n";
        }
            ...
            push( @header, "Taxonomy" );
            print { $annot_tab_file_handler{"tigrfam"} } "#",
              join( "\t", ( @common_header, @header ) ), "\n";
        }
            ...
            push( @header, "Taxonomy" );
            print { $annot_tab_file_handler{"casgene"} } "#",
              join( "\t", ( @common_header, @header ) ), "\n";
        }
            ...
            push( @header, "Taxonomy" );
            print { $annot_tab_file_handler{"metabolic"} } "#",
              join( "\t", ( @common_header, @header ) ), "\n";
        }
            ...
            push( @header, "Taxonomy" );
            print { $annot_tab_file_handler{"genomedb"} } "#",
              join( "\t", ( @common_header, @header ) ), "\n";
        }
        elsif ( $type eq "ec" ) {
            ...
            push( @header, "ec_name" );
            push( @header, "Taxonomy" );
            print { $annot_tab_file_handler{"ec"} } "#",
              join( "\t", ( @common_header, @header ) ), "\n";
        }
        elsif ( $type eq "ko" ) {
            ...
            push( @header, "Taxonomy" );
            print { $annot_tab_file_handler{"ko"} } "#",
              join( "\t", ( @common_header, @header ) ), "\n";
            ...
            push( @header, "go_id" );
            push( @header, "Taxonomy" );
            print { $annot_tab_file_handler{"go"} } "#",
              join( "\t", ( @common_header, @header ) ), "\n";
        ...
        keys %$seqHash )
    {
            ...
            @{ $seqHash->{$sid}{FEATURE} } )
        {
            ...
            my $tm =
              $f->has_tag("tm_num") ? ( $f->get_tag_values("tm_num") )[0] : "0";
            my $oc =
              $f->has_tag("genomedb_OC")
              ? ( $f->get_tag_values("genomedb_OC") )[0]
              : "unknow";
            my $score = $f->score;
            my $taxon = $oc;
            $taxon =~ s/\n//;
...
#@element_header = ("Gene", "Contig", "Start", "End", "Gene_Length", "Strand", "Feature");
            ...
            if ( exists $annot_tab_file_handler{"crispr"}
                && $type eq "repeat_region" )
            {
                my $count = $f->get_tag_values("rpt_unit_seq");
                for ( my $i = 0 ; $i < $count ; $i++ ) {
                    my $rpt_unit_seq =
                      ( $f->get_tag_values("rpt_unit_seq") )[$i];
                    my @row = (
                        $featureid, $seqid, $start,
                        $end,       $len,   $strand,
                        $type,      $score, $rpt_unit_seq
                    );
                    print { $annot_tab_file_handler{"crispr"} }
                      join( "\t", @row ), "\n";
                    $crispr_datatable_str .= $datatable_str;
                    $crispr_datatable_str .=
                      ( ' ' x 2 ) . "\"numRepeat\": \"$score\",\n";
                    $crispr_datatable_str .=
                      ( ' ' x 2 ) . "\"rpt_unit_seq\": \"$rpt_unit_seq\"\n";
            ...
            if ( exists $annot_tab_file_handler{"tRNA"} && $type eq "tRNA" ) {
                my $count =
                  $f->has_tag("name") ? $f->get_tag_values("name") : 0;
                for ( my $i = 0 ; $i < $count ; $i++ ) {
                    my $name = ( $f->get_tag_values("name") )[$i];
                    my @row  = (
                        $featureid, $seqid, $start, $end, $len, $strand, $type,
                        $name
                    );
                    print { $annot_tab_file_handler{"tRNA"} }
                      join( "\t", @row ), "\n";
            ...
            if ( exists $annot_tab_file_handler{"sprot"}
                && $f->has_tag("sprot_target") )
            {
            ...
            if ( exists $annot_tab_file_handler{"pfam"}
                && $f->has_tag("pfam_acc") )
            {
            ...
            if ( exists $annot_tab_file_handler{"tigrfam"}
                && $f->has_tag("tigrfam_acc") )
            {
                my $count = $f->get_tag_values("tigrfam_acc");
                for ( my $i = 0 ; $i < $count ; $i++ ) {
                    my $acc = ( $f->get_tag_values("tigrfam_acc") )[$i];
                    my $desc =
                      "\"" . ( $f->get_tag_values("tigrfam_desc") )[$i] . "\"";
                    my @row = (
                        $featureid, $seqid,  $start, $end,
                        $len,       $strand, $sp,    $tm,
                        $acc,       $desc,   $taxon
            ...
            if ( exists $annot_tab_file_handler{"casgene"}
                && $f->has_tag("casgene_acc") )
            {
            ...
            if ( exists $annot_tab_file_handler{"metabolic"}
                && $f->has_tag("metabolic_acc") )
            {
                ...
                for ( my $i = 0 ; $i < $count ; $i++ ) {
                    ...
                    my $process = "\""
                      . ( $f->get_tag_values("metabolic_process") )[$i] . "\"";
                    my @row = (
                        $featureid, $seqid,   $start, $end,
                        $len,       $strand,  $sp,    $tm,
                        $acc,       $process, $taxon
            ...
            if ( exists $annot_tab_file_handler{"genomedb"}
                && $f->has_tag("genomedb_acc") )
            {
                my $count = $f->get_tag_values("genomedb_acc");
                for ( my $i = 0 ; $i < $count ; $i++ ) {
                    my $acc = ( $f->get_tag_values("genomedb_acc") )[$i];
                    my @row = (
                        $featureid, $seqid, $start, $end, $len,
                        $strand,    $sp,    $tm,    $acc, $taxon
                    );
                    print { $annot_tab_file_handler{"genomedb"} }
                      join( "\t", @row ), "\n";
                }
            ...
            if ( exists $annot_tab_file_handler{"ec"}
                && $f->has_tag("allec_ids") )
            {
                ...
                for ( my $i = 0 ; $i < $count ; $i++ ) {
                    ...
                    print { $annot_tab_file_handler{"ec"} } join( "\t", @row ),
                      "\n";
                    print { $annot_tab_file_handler{"gene2ec"} }
                      "$featureid\t$ec\n";
                    $ec2genes{$ec}->{$featureid} = 1;
                }
            }

            if ( exists $annot_tab_file_handler{"ko"}
                && $f->has_tag("allko_ids") )
            {
                ...
                for ( my $i = 0 ; $i < $count ; $i++ ) {
                    ...
                    print { $annot_tab_file_handler{"ko"} } join( "\t", @row ),
                      "\n";
                    print { $annot_tab_file_handler{"gene2ko"} }
                      "$featureid\t$ko\n";
                    $ko2genes{$ko}->{$featureid} = 1;

                }
            }
            if ( exists $annot_tab_file_handler{"go"}
                && $f->has_tag("allgo_ids") )
            {
...
sub predict_kegg_pathways {
    ...
    my $cmd =
"python3 $ENV{'MinPath'}/MinPath.py -ko $prefix.mapping.txt -report $prefix.minpath -details $prefix.minpath.details ";
    ...
    open( INPUT, "$prefix.minpath.details" )
      or die "Could not find $prefix.minpath.details to read\n";
    $/ = "\npath";
    ...
    while (<INPUT>) {
        ...
        foreach my $item (@items) {
            $item =~ s/^\s+//;
            if ( my ( $id, $total_family_num, $total_family_found, $name ) =
                $item =~
                /(\d+)\s+fam0\s+(\d+)\s+fam-found\s+(\d+)\s+\#\s+(\S.*?)$/ )
            {
                $pid                         = $id;
            ...
            elsif ( my ( $ko, $geneCount, $koname ) =
                $item =~ /^(\S+)\s+hits\s+(\d+?)\s+\#?\s+(\S.*)$/ )
            {
    ...
    close(INPUT);
    $/ = "\n";
    open( GENE2PATHWAY, ">$datadir/cds.gene2kegg.tab.txt" )
      or die "Could not open $datadir/cds.gene2kegg.tab.txt to write, $!\n";
        ...
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            ...
            if ( exists $gene2pathways{$geneid}->{KEGG} ) {
                ...
                foreach my $pid ( keys %{ $gene2pathways{$geneid}->{KEGG} } ) {
                    ...
                    $f->add_tag_value( "kegg_pathway_name",
                        $pathways{$pid}->{pname} );

                    print GENE2PATHWAY
                      "$geneid\t$pid\t\"$pathways{$pid}->{pname}\"\t\"$oc\"\n";
    ...
    foreach my $pid ( keys %pathways ) {
        my $tn =
          exists $pathways{$pid}->{fam_all} ? $pathways{$pid}->{fam_all} : 0;
        my $tfn =
          exists $pathways{$pid}->{fam_found}
          ? $pathways{$pid}->{fam_found}
          : 0;
        my $pname =
          exists $pathways{$pid}->{pname} ? $pathways{$pid}->{pname} : "";
        my @kos = grep !/fam_all|fam_found|pname/,
          sort keys %{ $pathways{$pid} };
...
sub predict_metacyc_pathways {
    ...
    open( DATA, "$txtdir/MetaCyc_pathways_id2name.tab.txt" )
      or die
      "Could not open $txtdir/MetaCyc_pathways_id2name.tab.txt file to read\n";
    ...
    my $cmd =
"python3 $ENV{'MinPath'}/MinPath.py -any $prefix.mapping.txt -map $ENV{'MinPath'}/data/ec2path -report $prefix.minpath -details $prefix.minpath.details;";
    ...
    while (<INPUT>) {
        ...
        foreach my $item (@items) {
            $item =~ s/^\s+//;
            if ( my ( $total_family_num, $total_family_found, $id ) =
                $item =~ /fam0\s+(\d+)\s+fam-found\s+(\d+)\s+\#\s+(\S.*?)$/ )
            {
                $pid                         = $id;
                $pathways{$pid}->{fam_all}   = $total_family_num;
                $pathways{$pid}->{fam_found} = $total_family_found;
    ...
    close(INPUT);
    $/ = "\npath";
    open( GENE2PATHWAY, ">$datadir/cds.gene2metacyc.tab.txt" )
      or die "Could not open $datadir/cds.gene2metacyc.tab.txt to write, $!\n";
    ...
    foreach my $pid ( keys %pathways ) {
        ...
        my $pname =
          exists $pid2info{$pid}->{pname} ? $pid2info{$pid}->{pname} : "";
        my $ptype =
          exists $pid2info{$pid}->{ptype} ? $pid2info{$pid}->{ptype} : "";
        $pathways{$pid}->{ptype} = $ptype;
        $pathways{$pid}->{pname} = $pname;

        my @ecs = grep !/fam_all|fam_found|ptype|pname/,
          sort keys %{ $pathways{$pid} };
        foreach my $ec (@ecs) {
...
sub output_profiles {
    ...
    for my $sid ( keys %$seqHash ) {
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            ...
            my $mdepth_col_count =
                $f->has_tag("mdepth_cols")
              ? $f->get_tag_values("mdepth_cols")
              : -1;
            my %sample2depth = ();
...
#my $mainrole = $f->has_tag("tigrfam_mainrole") ? ($f->get_tag_values("tigrfam_mainrole"))[$i] : "";
                ...
                if ( $f->has_tag("allko_ids") ) {
                    ...
                    for ( my $i = 0 ; $i < $ko_count ; $i++ ) {
                        ...
                        my $de   = "";
                        my $stmt =
                          "select name,de from kos where koid=\"$koid\"";
                        my $sth = $dbh->prepare($stmt);
                        $sth->execute();
                        my $all = $sth->fetchall_arrayref();
                        foreach my $row (@$all) {
                    ...
                    for ( my $i = 0 ; $i < $acc_count ; $i++ ) {
                       ...
                       #compound:Sulfur;process:thiosulfate oxidation;gene:soxY;
                        my $desc =
                          ( $f->get_tag_values("metabolic_process") )[$i];
                        my ( $compound, $proc ) =
                          $desc =~ /^compound:(\S.*?);process:(\S+.*?).gene:/;
                ...
                if ( $f->has_tag("kegg_pathway_id") ) {
                    ...
                    for ( my $i = 0 ; $i < $pathway_count ; $i++ ) {
                        ...
                        my $pname =
                          ( $f->get_tag_values("kegg_pathway_name") )[$i];
                ...
                if ( $f->has_tag("metacyc_pathway_id") ) {
                    my $pathway_count =
                      $f->get_tag_values("metacyc_pathway_id");
                    for ( my $i = 0 ; $i < $pathway_count ; $i++ ) {
                        my $pid =
                          ( $f->get_tag_values("metacyc_pathway_id") )[$i];
    ...
    output_profile( \%stats, \%tigrfam2genes, "cds", "tigrfam", $outdir,
        \%tigrfam );
    output_profile( \%stats, \%metabolic2genes, "cds", "metabolic", $outdir,
        \%metabolic );
    ...
    output_pathway_profile( \%stats, \%keggPathway2genes, "cds", "kegg",
        $outdir, $keggs );
    output_pathway_profile( \%stats, \%metacycPathway2genes, "cds", "metacyc",
        $outdir, $metacycs );
...
###########SSU
    my $cmd =
"$^X $bin/output_tree_json.pl -i $outdir/taxon.ssu.profile.tab.txt -t Taxonomy> $outdir/taxon.ssu.tree.json;";
    $cmd .=
"$^X $bin/output_sunburst_json.pl -i $outdir/taxon.ssu.profile.tab.txt -t Taxonomy -p $outdir/taxon.ssu.profile.sunburst;";
    if ( @msamples > 0 ) {
        $cmd .=
"$^X $bin/output_sunburst_json.abund.pl -i $outdir/taxon.ssu.profile.tab.txt -t Taxonomy -p $outdir/taxon.ssu.profile.sunburst";
    }
...
##########LSU
    $cmd =
"$^X $bin/output_tree_json.pl -i $outdir/taxon.lsu.profile.tab.txt -t Taxonomy> $outdir/taxon.lsu.tree.json;";
    $cmd .=
"$^X $bin/output_sunburst_json.pl -i $outdir/taxon.lsu.profile.tab.txt -t Taxonomy -p $outdir/taxon.lsu.profile.sunburst;";
    if ( @msamples > 0 ) {
        $cmd .=
"$^X $bin/output_sunburst_json.abund.pl -i $outdir/taxon.lsu.profile.tab.txt -t Taxonomy -p $outdir/taxon.lsu.profile.sunburst";
    }
    msg("******start running $cmd\n");
    runcmd("$cmd");
    msg("******Finish running $cmd \n\n");
...
##########CDS
    $cmd =
"$^X $bin/output_tree_json.pl -i $outdir/taxon.cds.profile.tab.txt -t Taxonomy> $outdir/taxon.cds.tree.json;";
    $cmd .=
"$^X $bin/output_sunburst_json.pl -i $outdir/taxon.cds.profile.tab.txt -t Taxonomy -p $outdir/taxon.cds.profile.sunburst;";
    if ( @msamples > 0 ) {
        $cmd .=
"$^X $bin/output_sunburst_json.abund.pl -i $outdir/taxon.cds.profile.tab.txt -t Taxonomy -p $outdir/taxon.cds.profile.sunburst";
    }
...
###########metabolic
    $cmd =
"$^X $bin/output_tree_json.pl -i $outdir/metabolic.cds.profile.tab.txt -t Compound-process-gene> $outdir/metabolic.cds.profile.tree.json;";
    $cmd .=
"$^X $bin/output_sunburst_json.pl -i $outdir/metabolic.cds.profile.tab.txt -t Compound-process-gene -p $outdir/metabolic.cds.profile.sunburst;";
    if ( @msamples > 0 ) {
        $cmd .=
"$^X $bin/output_sunburst_json.abund.pl -i $outdir/metabolic.cds.profile.tab.txt -t  Foam -p $outdir/metabolic.cds.profile.sunburst";
    }

    msg("******start running $cmd\n");
    runcmd("$cmd");
    msg("******Finish running $cmd \n\n");
    output_treecol( "$outdir/metabolic.cds.treecol.json",
        "Metabolic_genes", \@msamples );
...
sub output_profile {
    ...
    my $total_depth =
      exists $stats{$feature}->{totalAvgDepth}
      ? $stats{$feature}->{totalAvgDepth}
      : 0;
    my @msamples = grep !/totalCount|totalAvgDepth/,
      sort keys %{ $stats{$feature} };
    ...
    open my $data_tab, ">", "$outdir/$attr.$feature.profile.tab.txt"
      or die
      "Could not open $outdir/$attr.$feature.profile.tab.txt to write, $!\n";
    print $data_tab join( "\t", @header ), "\n";

    open my $data_json, ">", "$outdir/$attr.$feature.profile.datatable.json"
      or die
"Could not open $outdir/$attr.$feature.profile.datatable.json to write, $!\n";
    ...
    for my $key ( keys %data ) {
        ...
        if ( defined $iddesc ) {
            if ( $attr eq "pfam" ) {
                my $id =
                  exists $iddesc->{$key}->{ID} ? $iddesc->{$key}->{ID} : "";
                my $desc =
                  exists $iddesc->{$key}->{Desc} ? $iddesc->{$key}->{Desc} : "";
                push( @cols, ( $id, "\"$desc\"" ) );
                ...
                my $name =
                  exists $iddesc->{$key}->{Name} ? $iddesc->{$key}->{Name} : "";
                my $desc =
                  exists $iddesc->{$key}->{Desc} ? $iddesc->{$key}->{Desc} : "";
                push( @cols, ( $name, "\"$desc\"" ) );
                ...
                my $name =
                  exists $iddesc->{$key}->{name} ? $iddesc->{$key}->{name} : "";
                my $de =
                  exists $iddesc->{$key}->{de} ? $iddesc->{$key}->{de} : "";
                $name =~ s/\""/\\\"/g;
                $de   =~ s/\""/\\\"/g;
                ...
                my $compound =
                  exists $iddesc->{$key}->{Compound}
                  ? $iddesc->{$key}->{Compound}
                  : "";
                my $process =
                  exists $iddesc->{$key}->{Process}
                  ? $iddesc->{$key}->{Process}
                  : "";
                my $desc =
                  exists $iddesc->{$key}->{Desc} ? $iddesc->{$key}->{Desc} : "";
        ...
        my $totalAvgDepth =
          exists $data{$key}->{totalAvgDepth}
          ? sprintf( "%.2f", $data{$key}->{totalAvgDepth} )
          : 0;
        my $totalAvgDepth_pct =
          $total_depth != 0
          ? sprintf( "%.2f", 100 * $totalAvgDepth / $total_depth )
          : 0;
        push( @cols,
            ( $count, $count_pct, $totalAvgDepth, $totalAvgDepth_pct ) );
        ...
        #print STDERR "****************$name, $feature, $abund, $total_abund\n";
            my $abund_pct =
              $total_abund > 0
              ? sprintf( "%.2f", 100 * $abund / $total_abund )
              : 0;
...
sub output_treecol {
    ...
    open( TREECOL, ">$outFile" )
      or die "Could not open $outFile to write, $!\n";
    ...
    for ( ; $i < $size ; $i++ ) {
        ...
        if ( $i == $size - 1 ) {
            print TREECOL
"{width: \"100px\", value: \"$key\", header: \"$treecol{$key}\"}\n"
...
sub output_pathway_profile {
    ...
    my @header = ( "Count", "Count_pct", "Abund", "Abund_pct" );
    unshift( @header,
        "#Pathway_id", "Name", "KOs", "total_family", "total_family_found" )
      if ( $attr =~ /kegg/ );
    unshift( @header,
        "#Pathway_id", "Type",         "Name",
        "ECs",         "total_family", "total_family_found" )
      if ( $attr =~ /metacyc/ );
...
"Could not open $outdir/$attr.$feature.profile.datatable.json to write, $!\n";
    ...
    for my $key ( keys %data ) {
        ...
        if ( defined $iddesc ) {
            if ( $attr eq "kegg" ) {
                my $name =
                  exists $iddesc->{$key}->{pname}
                  ? $iddesc->{$key}->{pname}
                  : "";
                my $fam_all =
                  exists $iddesc->{$key}->{fam_all}
                  ? $iddesc->{$key}->{fam_all}
                  : "";
                my $fam_found =
                  exists $iddesc->{$key}->{fam_found}
                  ? $iddesc->{$key}->{fam_found}
                  : "";
                my @kos = grep !/pname|fam_found|fam_all/,
                  sort keys %{ $keggs->{$key} };
                my $kos_str = join( "+", @kos );
                ...
                my @ecs = grep !/pname|fam_found|fam_all|ptype/,
                  sort keys %{ $metacycs->{$key} };
                my $ecs_str = join( "+", @ecs );

                push(
                    @cols,
                    (
                        "\"$type\"", "\"$name\"", $ecs_str, $fam_all,
                        $fam_found
                    )
                );
...
"return \'<a href=\"https://www.genome.jp/dbget-bin/www_bget?pathway+\' + pid + \'\">\' + pid + \'</a>\';\n";
        print $jsonFileHandler ( ' ' x 2 ), "}\n";
        ...
        print $jsonFileHandler ( ' ' x 6 ),
"return \'<a href=\"https://www.genome.jp/kegg-bin/show_pathway?\' + pid + \'+\' + kos + \'\">\' + data + \'</a>\';\n";
        print $jsonFileHandler ( ' ' x 4 ), "}\n";
        ...
        print $jsonFileHandler ( ' ' x 2 ),
          "\"render\": function(data,type,full,meta){\n";
        print $jsonFileHandler ( ' ' x 4 ),
"return \'<a href=\"https://biocyc.org/META/NEW-IMAGE?type=PATHWAY&object=\' + data + \'\">\' + data + \'</a>\';\n";
        print $jsonFileHandler ( ' ' x 2 ), "}\n";
        print $jsonFileHandler "},\n";
        ...
        print $jsonFileHandler ( ' ' x 2 ),
          "\"render\": function(data,type,full,meta){\n";
        print $jsonFileHandler ( ' ' x 4 ),
"return \'<a href=\"https://pfam.xfam.org/family/' + data + \'\">\' + data + \'</a>\';\n";
        print $jsonFileHandler ( ' ' x 2 ), "}\n";
        ...
        print $jsonFileHandler ( ' ' x 2 ),
          "\"render\": function(data,type,full,meta){\n";
        print $jsonFileHandler ( ' ' x 4 ),
"return \'<a href=\"http://tigrfams.jcvi.org/cgi-bin/HmmReportPage.cgi?acc=' + data + \'\">\' + data + \'</a>\';\n";
        print $jsonFileHandler ( ' ' x 2 ), "}\n";
        print $jsonFileHandler "},\n";
        ...
        print $jsonFileHandler ( ' ' x 2 ),
          "\"render\": function(data,type,full,meta){\n";
        print $jsonFileHandler ( ' ' x 4 ),
"return \'<a href=\"https://www.genome.jp/dbget-bin/www_bget?ko\:' + data + \'\">\' + data + \'</a>\';\n";
        print $jsonFileHandler ( ' ' x 2 ), "}\n";
        ...
        print $jsonFileHandler ( ' ' x 2 ),
          "\"render\": function(data,type,full,meta){\n";
        print $jsonFileHandler ( ' ' x 4 ),
"return \'<a href=\"https://enzyme.expasy.org/EC/' + data + \'\">\' + data + \'</a>\';\n";
        print $jsonFileHandler ( ' ' x 2 ), "}\n";
        ...
        print $jsonFileHandler ( ' ' x 2 ),
          "\"render\": function(data,type,full,meta){\n";
        print $jsonFileHandler ( ' ' x 4 ),
"return \'<a href=\"http://amigo.geneontology.org/amigo/term/' + data + \'\">\' + data + \'</a>\';\n";
        print $jsonFileHandler ( ' ' x 2 ), "}\n";
    ...
    if (@msamples) {
        ...
        for my $i ( 0 .. $#msamples ) {
            ...
            print $jsonFileHandler ( ' ' x 2 ),
              "\"title\": \"Abund\_$msamples[$i]\",\n";
            print $jsonFileHandler ( ' ' x 2 ),
              "\"data\": \"abund\_$msamples[$i]\"\n";
            print $jsonFileHandler "},\n";
...
sub output_gff {
        ...
        keys %$seqHash )
    {
            ...
            @{ $seqHash->{$sid}{FEATURE} } )
        {
            ...
            foreach my $ftag ( $f->all_tags() ) {
                if ( $ftag =~
/foam_ecs|foam_kos|foam_target|sprot_kegg|sprot_kos|sprot_ec|sprot_go|pfam_go|tigrfam_ec|tigrfam_mainrole|tigrfam_sub1role|_target/
                  )
                {
                    $f->remove_tag($ftag);
                }
