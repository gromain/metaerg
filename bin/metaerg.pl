...
my (
    @Options,    $quiet,             $debug,        $force,
    $prefix,     $outdir,            $locustag,     $increment,
    $gcode,      $gtype,             $mincontiglen, $minorflen,
    $evalue,     $cpus,              $identity,     $coverage,
    $hmm_cutoff, $hmm_evalue_cutoff, $sp,           $tm,
    $depth_f,    $DBDIR
);
...
if ( -d $outdir ) {
    if ($force) {
        msg("Re-using existing --outdir $outdir");
          ...
          err (
"Folder '$outdir' already exists! Please change --outdir or use --force"
          );
    }
}
else {
...
my $param =
"$EXE --dbdir $DBDIR --mincontiglen $mincontiglen --minorflen $minorflen --prefix $prefix --outdir $outdir --locustag $locustag --increment $increment --cpus $cpus --evalue $evalue --identity $identity --coverage $coverage -hmmcutoff $hmm_cutoff -hmmevalue $hmm_evalue_cutoff";
...
my $cmd =
"$^X $bin/predictFeatures.pl --dbdir $DBDIR --evalue $evalue --gtype $gtype --gc $gcode --minorflen $minorflen --prefix $prefix --cpus $cpus --outdir $tmpdir ";
$cmd .= "--sp " if ($sp);
$cmd .= "--tm " if ($tm);
$cmd .= "--force $fasta";
runcmd("$cmd");
...
$cmd =
"$^X $bin/annotCDs.pl --dbdir $DBDIR --cpus $cpus --evalue $evalue --identity $identity --coverage $coverage --hmmcutoff $hmm_cutoff --hmmevalue $hmm_evalue_cutoff --outdir $tmpdir $faa";
runcmd("$cmd");
msg("******Finish annotate cds\n\n");

my %seqHash = ();

open my $gff, "$tmpdir/features.annot.gff"
  or die "could not open $tmpdir/features.annot.gff to read, $!\n";
...
#open (MAP, ">idmap.txt");
for my $sid ( sort { $seqHash{$b}{DNA}->length <=> $seqHash{$a}{DNA}->length }
    keys %seqHash )
{
    ...
    {
        next
          unless $f->primary_tag =~
          m/CDS|RNA|signal_peptide|repeat_region|transmembrane_helix/;
        $num_lt++;
...
$cmd =
"$^X $bin/output_reports.pl -g $datadir/all.gff -o $outdir -f $outdir/$prefix.fna -db $DBDIR";
...
sub get_all_kos {
    ...
    for my $sid ( keys %$seqHash ) {
        ...
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            ...
            my $foam_kos = ( $f->get_tag_values('foam_kos') )[0]
              if ( $f->has_tag('foam_kos') );
            my $sprot_kos = ( $f->get_tag_values('sprot_kos') )[0]
              if ( $f->has_tag('sprot_kos') );
...
sub get_all_ecs {
    ...
    for my $sid ( keys %$seqHash ) {
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            my $featureid = ( $f->get_tag_values("ID") )[0];
            next unless $f->primary_tag eq "CDS";
            my $foam_ec = ( $f->get_tag_values('foam_ec') )[0]
              if ( $f->has_tag('foam_ec') );
            my $sprot_ec = ( $f->get_tag_values('sprot_ec') )[0]
              if ( $f->has_tag('sprot_ec') );
            my $tigrfam_ec = ( $f->get_tag_values('tigrfam_ec') )[0]
              if ( $f->has_tag('tigrfam_ec') );
            $foam_ec    //= "";
            $sprot_ec   //= "";
            $tigrfam_ec //= "";
...
sub get_all_gos {
    ...
    for my $sid ( keys %$seqHash ) {
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            ...
            my $pfam_go =
              $f->has_tag('pfam_go')
              ? ( $f->get_tag_values('pfam_go') )[0]
              : "";
            my $sprot_go =
              $f->has_tag('sprot_go')
              ? ( $f->get_tag_values('sprot_go') )[0]
              : "";
            my @all_gos = ();
        ...
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            ...
            if ( $f->has_tag('sprot_target') ) {
                ...
                if ( my ($spid) = $target =~ /db:uniprot_sprot\|(\S+)/ ) {
                    my $stmt =
                      "select * from uniprot_spid2annot where spid=\"$spid\"";
                    my $sth = $dbh->prepare($stmt);
                    $sth->execute();
                    my $all = $sth->fetchall_arrayref();

                    foreach my $row (@$all) {
                        my ( $spid, $ko, $kegg, $ec, $desc, $go, $os, $oc ) =
                          @$row;
        ...
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            ...
            if ( $f->has_tag('genomedb_target') ) {
                ...
                if ( my ($gid) = $target =~ /db:genomedb\|(\S+?)\|/ ) {
                    ...
                    my $stmt =
                      "select lineage from genome2taxon where gid=\"$gid\"";
            ...
            if ( $f->has_tag('pfam_target') ) {
                ...
                for my $target ( $f->get_tag_values('pfam_target') ) {
                    if ( my ($pfamid) = $target =~ /^db:\S+?\|(\w+).*/ ) {
                        my $stmt =
"SELECT acc, id, DE, TP FROM pfams where acc=\"$pfamid\"";
                        my $sth = $dbh->prepare($stmt);
                        $sth->execute();
                        my $all = $sth->fetchall_arrayref();
                        foreach my $row (@$all) {
                        ...
                        $stmt =
"SELECT go_acc from pfam2go where pfam_acc=\"$pfamid\"";
            ...
            if ( $f->has_tag('tigrfam_target') ) {
                for my $target ( $f->get_tag_values('tigrfam_target') ) {
                    if ( my ($tigrfamid) = $target =~ /^db:\S+?\|(\S+)/ ) {
                        my $stmt =
"SELECT acc,id,de,it,gs,ec,mainrole, sub1role FROM tigrfams where acc=\"$tigrfamid\"";
                        my $sth = $dbh->prepare($stmt);
                        $sth->execute();
                        my $all = $sth->fetchall_arrayref();
                        foreach my $row (@$all) {
                            my ( $acc, $id, $de, $it, $gs, $ec, $mainrole,
                                $sub1role )
                              = @$row;
                            $de       ||= "";
                            $it       ||= "";
                            ...
                            $mainrole ||= "";
                            $sub1role ||= "";
                            $f->add_tag_value( 'tigrfam_acc', $acc )
                              if $acc ne "";
                            $f->add_tag_value( 'tigrfam_desc', $de )
                              if $de ne "";

                            #$f->add_tag_value('tigrfam_it', $it) if $it ne "";
                            #$f->add_tag_value('tigrfam_gs', $gs) if $gs ne "";
                            $f->add_tag_value( 'tigrfam_ec', $ec ) if $ec ne "";
                            $f->add_tag_value( 'tigrfam_mainrole', $mainrole )
                              if $mainrole ne "";
                            $f->add_tag_value( 'tigrfam_sub1role', $sub1role )
                              if $sub1role ne "";
                            $f->add_tag_value( 'tigrfam_name', $id )
                              if $id ne "";
                        ...
                        $stmt =
"SELECT go_acc from tigrfam2go where tigrfam_acc=\"$tigrfamid\"";
                        $sth = $dbh->prepare($stmt);
                        $sth->execute();
                        $all = $sth->fetchall_arrayref();
                        my $gos = "";
...
sub mapping_metabolic {
    ...
    open( MMARKER, "$bin/../txt/metabolic.txt" )
      or die "could not open $bin/../txt/metabolic.txt to read, $!\n";
    my %hmm2process = ();
        ...
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
           ...
           #print STDERR "xiaoli********************************************\n";
                for my $target ( $f->get_tag_values('metabolic_target') ) {
                    ...
                    #print STDERR $target, "\n";
                    if ( my ( $metabolicfamName, $score ) =
                        $target =~ /^db:\S+?\|(\S+?)\s+.*?score:(\S+)/ )
                    {
#print STDERR "xiaoli if metabolicfamName=$metabolicfamName, score=$score********************************************\n";
                        if ( exists $hmm2process{$metabolicfamName} ) {
                            if ( not exists $metabolicfams{$metabolicfamName}
                                && exists $hmm2process{$metabolicfamName} )
                            {
                                my $cutoff_score =
                                  $hmm2process{$metabolicfamName}
                                  ->{cutoff_score};
                                print STDERR
"$metabolicfamName\t$score\tcutoff=$cutoff_score\n";
                                if (
                                    (
                                           $cutoff_score eq "-"
                                        || $cutoff_score <= $score
                                ...
                                {
                                    $f->add_tag_value( 'metabolic_process',
                                        "compound:"
                                          . $hmm2process{$metabolicfamName}
                                          ->{chemical}
                                          . ";process:"
    ...
    for my $sid ( keys %$seqHash ) {
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            ...
            if ( $f->has_tag('foam_target') ) {
                ...
                $f->add_tag_value( 'foam_ecs', join( ",", keys %ecs ) )
                  if keys %ecs > 0;
        ...
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            ...
            if ( $f->has_tag('casgene_target') ) {
                ...
                foreach my $target ( $f->get_tag_values('casgene_target') ) {
                    if ( my ( $acc, $full_score, $best_domain_score, $name ) =
                        $target =~
/db:casgenes.hmm\|(\S+?)\s.*?score:(\S+).*?best_domain_score:(\S+?)\s+name:(\S+)/
                      )
                    {
...
/foam_ecs|foam_kos|foam_target|sprot_kegg|sprot_kos|sprot_ec|sprot_go|pfam_go|tigrfam_ec|tigrfam_mainrole|tigrfam_sub1role|_target/
    ...
    my $fin =
      Bio::SeqIO->new( -file => "$outdir/tmp/cds.faa", -format => 'fasta' );
    while ( my $seq = $fin->next_seq ) {
    ...
    while (<FDEPTH>) {
        ...
        if (/^contigName/) {
            ...
            for my $i ( keys %headerIndex2SampleName ) {
                $contig2depth{$contigName}->{ $headerIndex2SampleName{$i} } =
                  $l[$i];
            }
        }
    }
...
sub setOptions {
    ...
    @Options = (
        ...
        },
        {
            ...
            VAR     => \$gtype,
            DEFAULT => "meta",
            DESC    => "single or metagenome: [arc|bac|euk|meta]"
        },
        {
            OPT     => "minorflen=i",
            VAR     => \$minorflen,
        ...
        },
        {
            ...
            DEFAULT => 0,
            DESC    =>
"Enable signal peptide and cleavage site predication using signalp, it is slow when it is enabled"
        },
        {
            OPT     => "tm!",
            VAR     => \$tm,
            DEFAULT => 0,
            DESC    =>
"Enable transmembrane helics predication using tmhmm, it is slow when it is enabled"
        },
        ...
        },
        {
            ...
            VAR     => \$increment,
            DEFAULT => 1,
            DESC    => "Locus tag counter increment"
        },
        ...
        'diamond cutoff:',
        {
            ...
            VAR     => \$evalue,
            DEFAULT => 1e-5,
            DESC    => "Similarity e-value cut-off"
        },
        {
            OPT     => "identity=f",
            VAR     => \$identity,
        ...
        'hmmsearch cutoff:',
        {
            ...
            DEFAULT => "--cut_ga",
            DESC    =>
              "hmm search trusted score threshold: [--cut_ga|--cut_nc|--cut_tc]"
        },
        {
            OPT     => "hmmevalue=f",
            VAR     => \$hmm_evalue_cutoff,
            DEFAULT => "1e-5",
            DESC    => "custom hmm db search threshold"
        },
        ...
        'abundance input:',
        {
            ...
            DEFAULT => "",
            DESC    =>
              "contig coverage depth file in format of:contigid contigLe depth"
        }
    );
...
"Uh oh! Sequence file '$contig' contains duplicate sequence ID:",
    ...
    $ncontig > 0
      or err ("FASTA file '$contig' contains no suitable sequence entries");
