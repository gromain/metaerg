...
my (
    @Options,    $outdir,            $evalue,
    $cpus,       $identity,          $coverage,
    $hmm_cutoff, $hmm_evalue_cutoff, $DBDIR
);
setOptions();
my $faa = shift @ARGV
  or err
  ("Please supply a fasta format coding  sequence file on the command line.");
...
push(
    @thrs,
    threads->new(
        \&cds_hmmerSearch_FOAM, $faa,
        "FOAM-hmm_rel1a.hmm",   $outdir,
        $cpus,                  "--cut_tc"
    )
);
push(
    @thrs,
    threads->new(
        \&cds_diamondSearch, $faa, "uniprot_sprot", $outdir,
        $cpus, $evalue, 1
    )
);
push(
    @thrs,
    threads->new(
        \&cds_diamondSearch, $faa, "genomedb", $outdir, $cpus, $evalue, 0
    )
);
push(
    @thrs,
    threads->new(
        \&cds_hmmerSearch, $faa,        "Pfam-A.hmm", $outdir,
        $cpus,             $hmm_cutoff, $hmm_evalue_cutoff
    )
);
push(
    @thrs,
    threads->new(
        \&cds_hmmerSearch, $faa,        "TIGRFAMs.hmm", $outdir,
        $cpus,             $hmm_cutoff, $hmm_evalue_cutoff
    )
);
...
push(
    @thrs,
    threads->new(
        \&cds_hmmerSearch, $faa, "metabolic.hmm", $outdir,
        $cpus, "", $hmm_evalue_cutoff
    )
);
push(
    @thrs,
    threads->new(
        \&cds_hmmerSearch, $faa, "casgenes.hmm", $outdir,
        $cpus, "", $hmm_evalue_cutoff
    )
);
...
open my $gff, "$outdir/features.gff"
  or die "could not open $outdir/features.gff to read, $!\n";
my @seqArray = ();
...
parse_search_results( $outdir, \%seqHash, $coverage, $identity,
    $hmm_evalue_cutoff );
...
sub cds_diamondSearch {
    ...
    my $cmd =
"diamond blastp -k 1 --quiet --masking $mask -p $cpus -q $faa -d $db -e $evalue -f 6 qseqid sseqid qlen qstart qend sstart send qframe pident bitscore evalue qcovhsp > $blstable ";
...
# remove 2> /dev/null at the end
    ...
    msg(    "diamond blastp search using $dbname database took:"
          . timestr($td)
          . " to run\n" );
}
...
sub cds_hmmerSearch_FOAM {
    ...
    push(
        @thrs,
        threads->new(
            \&cds_hmmerSearch, $faa,  "FOAM1.hmm",
            $outdir,           $cpus, $hmm_cutoff
        )
    );
    push(
        @thrs,
        threads->new(
            \&cds_hmmerSearch, $faa,  "FOAM2.hmm",
            $outdir,           $cpus, $hmm_cutoff
        )
    );
    push(
...
#my @db_subset = ("FOAM1", "FOAM2","FOAM3","FOAM4","FOAM5", "FOAM6","FOAM7","FOAM8", "FOAM9");
    ...
    my $td = timediff( $t1, $t0 );
    msg(
        "hmmsearch using $dbname database took:" . timestr($td) . " to run\n" );
}
...
#my $cmd = "hmmsearch --notextw --acc $hmm_cutoff --cpu $cpus $db $faa  > \Q$hmmout\E 2> /dev/null";
    ...
    if ( $hmm_cutoff =~ /\S+/ ) {
        $cmd =
"hmmsearch --notextw --acc $hmm_cutoff --cpu $cpus --tblout \Q$hmmout\E $db $faa ";

        # removed > /dev/null 2>&1 at the end
    }
    else {
        $cmd =
"hmmsearch --notextw --acc -E $hmm_evalue_cutoff --domE $hmm_evalue_cutoff --incE  $hmm_evalue_cutoff  --incdomE $hmm_evalue_cutoff --cpu $cpus --tblout \Q$hmmout\E $db $faa";
