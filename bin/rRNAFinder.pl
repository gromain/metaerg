...
my (
    @Options, $quiet, $threads,    $outdir,   $evalue,
    $domain,  $len,   $identities, $coverage, $DBDIR
);
setOptions();
...
foreach my $kdom (@domains) {
    ...
    my $cmd =
"nhmmer --cpu $threads -E $evalue -o /dev/null --tblout $search_output \Q$hmmdb\E \Q$fasta\E";
...
#for each location, only the best hit (lowest evalue and highest score) is reported
    open( OUT, $search_output )
      or die "Could not open $search_output to read, $!\n";
    foreach (<OUT>) {
        chomp;
        ...
        my @line = split( /\s+/, $_ );
          err ("bad line in nhmmer output - @line")
          unless defined $line[6] and $line[6] =~ m/^\d+$/;
        my ( $alifrom, $alito, $strand ) =
          $line[6] < $line[7]
          ? ( $line[6], $line[7], '+' )
          : ( $line[7], $line[6], '-' );
        my ( $seqid, $qname, $evalue, $score ) =
          ( $line[0], $line[2], $line[12], $line[13] );
        ...
        if ( exists $read2hmmhit{$seqid}->{$loc} ) {
            if (   $read2hmmhit{$seqid}->{$loc}->{evalue} > $evalue
    ...
    foreach my $loc1 ( keys %{ $read2hmmhit{$seqid} } ) {
        foreach my $loc2 ( keys %{ $read2hmmhit{$seqid} } ) {
            ...
            if ( is_overlapping( $s1, $e1, $s2, $e2 ) ) {
                ...
                if ( $read2hmmhit{$seqid}->{$loc1}->{evalue} >
                       $read2hmmhit{$seqid}->{$loc2}->{evalue}
                    || $read2hmmhit{$seqid}->{$loc1}->{score} <
                    $read2hmmhit{$seqid}->{$loc2}->{score} )
                {
                    push( @dels, $loc1 );
                }
                else {
...
foreach my $seqid ( keys %read2hmmhit ) {
    ...
    foreach my $loc ( keys %{ $read2hmmhit{$seqid} } ) {
        ...
        if ( $qname =~ m/(16S|18S|23S|28S)/ && $fragLen < $len ) {
            msg(
"Skipping short rRNA: len=$fragLen $seqid\t$kdom\t$qname\t$alifrom\t$alito\t$evalue\t$strand\t$score\n"
            );
            next;
        }
        else {
            ...
            my $gff_out_string =
"$seqid\t$EXE\t$qname\t$alifrom\t$alito\t$evalue\t$strand\t\.\tID=$ID;Name=$qname;score=$score\n";
            $rna2gff{$seqid}->{$ID} = $gff_out_string;
        }
    }
}

open( GFF, ">$outdir/rRNA.gff" )
  or die "Could not open $outdir/rRNA.gff to write, $!\n";
...
sub rna2taxon {
    ...
    foreach my $cat ( sort keys %$rnatcats ) {
        ...
        my $cmd =
"$^X $bin/rna2taxon.pl --dbdir $DBDIR/blast --cpus $threads --dbtype $dbtype --evalue $evalue --identities $identities --coverage $coverage $outdir/$input > $outdir/$output";
        msg("Command: $cmd");
        system($cmd) >> 8 and die "Could not execute cmd=$cmd, $!\n";
...
#9735487 rRNAFinder.pl   bac_16SrRNA        2       66      3.3e-12 -       .       ID=9735487_1;Name=bac_16SrRNA
    ...
    foreach my $cat ( keys %$rnacats ) {
        ...
        open( $file_handles{$cat}, ">$outdir/$cat" . ".ffn" )
          or die "Could not open $outdir/$cat" . ".ffn to write, $!\n";
    }
    $/ = "\n>";
    while (<FASTA>) {
        chomp;
            ...
            if ( exists $rnas{$sid} ) {
                ...
                foreach my $rnatype ( keys %{ $rnas{$sid} } ) {
                    ...
                    foreach my $loc ( keys %{ $rnas{$sid}->{$rnatype} } ) {
                        my ( $start, $end, $strand, $id ) =
                          $loc =~ /(\d+)\:(\d+)\:(\S+)\:(\S+)/;
                        my $length = $end - $start + 1;
                        if (
                            ( $rnatype =~ /5S|5_8S/ )
                            || (   $rnatype =~ m/(16S|18S|23S|28S)/
                                && $length >= $len )
                          )
                        {
                            print { $file_handles{$cat} }
">$id type=$rnatype len=$length start=$start end=$end strand=$strand\n";
                            print { $file_handles{$cat} }
                              substr( $seq, $start - 1, $length ), "\n";
                        }
...
sub setOptions {
    ...
    @Options = (
        ...
        },
        {
            ...
            VAR     => \$threads,
            DEFAULT => 8,
            DESC    => "Number of threads/cores/CPUs to use"
        },

        {
            ...
            VAR     => \$evalue,
            DEFAULT => 1E-09,
            DESC    => "evalue cut-off for gene prediction and taxon assignment"
        },
        {
            OPT     => "domain=s",
            VAR     => \$domain,
            DEFAULT => 'meta',
            DESC    => "Search rRNA for rRNA in domains: [arc|bac|euk|meta]"
        },
        ...
        'rRNA gene prediction options',
        {
            ...
            VAR     => \$len,
            DEFAULT => 180,
            DESC    => "length cut-off for 16/18/23/28S rRNA only"
        },
        ...
        'rRNA gene taxon assignment options:',
        {
            ...
            DEFAULT => 80,
            DESC    =>
"The min percent of the query sequence ovelaping with the subject sequence"
        },
        {
            OPT     => "identities=s",
            VAR     => \$identities,
            DEFAULT => '70,80,85,90,95,97,99',
            DESC    =>
"the percent identity of a match must exceed the given value of percent identity to be assigned at the given rank:domain70, phylum 80, class 85, order 90, family 95, genus 97, species 99"
        }
    );
