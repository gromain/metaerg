...
my $VERSION = "0.1";
my $DESC =
  "ribosomal RNA classification: associate ribosomal RNA to SILVA taxonomy";
...
if ( @arr < 7 ) {
      err
      ("the number of the identity cutoff is less than 6, please redefine it");
}
...
my $cmd =
"blastn  -query $fasta -db $dbdir/$db -dust no -num_threads $cpus -evalue $evalue -out $blastn_output  -outfmt \"7 qseqid qlen slen qstart qend sstart send length qcovhsp pident evalue bitscore stitle\" -max_target_seqs 5";
...
open( BLAST_OUT, $blastn_output )
  or die "Could not open $blastn_output to read, $!\n";
...
# Fields: query id, query length, subject length, q. start, q. end, s. start, s. end, alignment length, % query coverage per hsp, % identity, evalue, bit score, subject title
    ...
    my $target =
"rRNA_target=db:$db|$ssid $fields[3] $fields[4] evalue:$fields[10] qcov:$fields[8] identity:$iden";
...
sub lca {
    ...
    if ( @taxons == 0 ) {
        die("undefined values snuck into the taxid list; help?");
    }    # how did undefined values get on the list?
...
sub setOptions {
    ...
    @Options = (
        ...
        },
        {
            ...
            VAR     => \$dbdir,
            DEFAULT => '',
            DESC    => "metaerg database directory"
        },
        {
            OPT     => "cpus=i",
            VAR     => \$cpus,
        ...
        },
        {
            ...
            DEFAULT => 'ssu',
            DESC    =>
"input sequence type:ssu|lsu, ssu incldues 16s/18s, lsu includes 23s/28s rRNA"
        },
        'Cutoffs:',
        {
            OPT     => "evalue=f",
        ...
        },
        {
            ...
            VAR     => \$length,
            DEFAULT => 200,
            DESC    => "rRNA gene length cutoff for taxon assignment"
        },
        {
            OPT     => "coverage=f",
            VAR     => \$coverage,
            DEFAULT => 80,
            DESC    =>
"The min query coverage: percent of the query sequence taht ovelaps the subject sequence"
        },
        {
            OPT     => "identities=s",
            VAR     => \$identities,
            ...
            DESC    =>
"the percent identity of a match must exceed the given value of percent identity to be assigned at the given rank:domain 70, phylum 80, class 85, order 90, family 95, genus 97, species 99"
        },
