...
sub parse_search_results {
    ...
    parse_diamond( $diamond_output, "uniprot_sprot", "sprot", $seqHash,
        $coverage, $identity );
    ...
    parse_diamond(
        $diamond_output, "genomedb", "genomedb", $seqHash,
        $coverage,       $identity
    );
    ...
    parse_tblout_hmmsearch( $hmmsearch_output, "TIGRFAMs.hmm", "tigrfam",
        $seqHash );
    ...
    parse_tblout_hmmsearch( $hmmsearch_output, "metabolic.hmm", "metabolic",
        $seqHash );
    ...
    parse_tblout_hmmsearch( $hmmsearch_output, "casgenes.hmm", "casgene",
        $seqHash );
    ...
    parse_tblout_hmmsearch( $hmmsearch_output, "FOAM-hmm_rel1a.hmm", "foam",
        $seqHash );
...
#qseqid sseqid qlen qstart qend sstart send qframe pident bitscore evalue qcovhsp
    open( F6, $diamond_output )
      or die "Could not open $diamond_output to read, !$\n";
    my $num_hit = 0;
    while (<F6>) {
        chomp;
        my @line = split( /\t/, $_ );
        ...
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            if ( ( $f->get_tag_values("ID") )[0] eq $line[0] ) {
                if ( $line[8] >= $identity && $line[11] >= $coverage ) {
                    $f->add_tag_value( "$type\_target",
"db:$dbname|$line[1] $line[3] $line[4] evalue:$line[10] qcov:"
                          . sprintf( "%.2f", $line[11] )
                          . " identity:"
                          . sprintf( "%.2f", $line[8] ) );
                    $num_hit++;
...
sub parse_tblout_hmmsearch {
    ...
    open( TBLOUT, $hmmsearch_output )
      or die "Could not open $hmmsearch_output to read, !$\n";
    my $num_hit = 0;
    ...
    while (<TBLOUT>) {
        ...
        for my $f ( @{ $seqHash->{$sid}{FEATURE} } ) {
            if ( ( $f->get_tag_values("ID") )[0] eq $cid ) {
                my $target_value =
"db:$dbname|$qacc evalue:$full_evalue score:$full_score best_domain_score:$best_domain_score name:$qname";

                #print STDERR "type=$type $target_value\n";
                next
                  if ( $dbname eq "casgenes.hmm"
                    && ( $full_score < 25 || $best_domain_score < 22 ) );
...
sub parse_hmmer3_hmmsearch {
    ...
    my $hmmer3 =
      Bio::SearchIO->new( -file => $hmmsearch_out, -format => $format );
...
#print STDERR "$qacc hitname=$hitname seqT=$seqT\n" if $dbname eq "metabolic.hmm";
                        while ( my $hsp = $hit->next_hsp ) {
                            ...
                            my $target_value =
                                "db:$dbname|$qacc $start $end evalue:$sig qcov:"
                              . sprintf( "%.2f", $qcov )
                              . " identity:"
                              . sprintf( "%.2f", 100 * $frac_id )
                              . " score:$score seqT:$seqT name:$qname";
                    ...
                    if ( ( $f->get_tag_values("ID") )[$i] eq $hitname ) {
                        ...
                        while ( my $hsp = $hit->next_hsp ) {
                            ...
                            if ( $f->has_tag("$type\_target") ) {
                                ...
                                my @values    = ();
                                foreach my $ptarget (
                                    $f->get_tag_values("$type\_target") )
                                {
                                    my ( $pstart, $pend, $pevalue, $pscore ) =
                                      $ptarget =~
/^\S+?\s+(\d+?)\s+(\d+?)\s+?evalue\:(\S+).*?score\:(\S+)/;
                                    if (
                                        is_overlapping(
