...
open( BINNED, ">$bindir/binned_concat.fasta" )
  or die "Cannot open $bindir/binned_concat.fasta to write: $!\n";
open( ANNOT, ">$bindir/binned_annotation.list" )
  or die "Cannot open $bindir/binned_annotation.list to write: $!\n";
print ANNOT "label\n";
...
foreach my $fasta (@dirdata) {
    ...
    open( FASTA, "$bindir/$fasta" )
      || die "Could not open $bindir/$fasta file to read, $!\n";
    $/ = "\n>";
    while (<FASTA>) {
        chomp;
        if ( my ( $seqid, $seq ) = /^>?(\S+).*?\n(.*)/s ) {
